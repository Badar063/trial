...
def load_results():
    ...
    if os.path.exists(RESULTS_FILE):
        ...
        st.error(f"⚠️ **Deployment Error:** Results file '{RESULTS_FILE}' not found. Please run your core Python script first to generate the data!")
...
st.title("🚗 ArchNet: The CNN Performance Atlas")
...
tab1, tab2 = st.tabs(["📋 Performance Atlas", "🤖 ArchNet Advisor"])
...
with tab1:
    ...
    if not results_df.empty:
        ...
        with col_summary_1:
            st.metric("🏆 Best F1-Score (Diagnostic Power)", best_accuracy['Architecture'], delta=best_accuracy['Accuracy'])
        with col_summary_2:
            st.metric("⚡ Fastest Inference (Operational Speed)", best_speed['Architecture'], delta=f"{best_speed['Inference Time (s)']}")
        with col_summary_3:
            st.metric("💾 Most Efficient (Smallest Size)", results_df.loc[results_df['Model Size (MB)'].idxmin()]['Architecture'], delta=f"{results_df['Model Size (MB)'].min()} MB")

# --- 2. ArchNet Advisor Tab ---
with tab2:
    st.header("🤖 ArchNet Advisor: Optimal Model Selector")
    ...
    with st.form("advisor_form"):
        ...
        priority_options = {
            'accuracy': '🎯 Maximum Accuracy (Critical care, low tolerance for error)',
            'speed': '⚡ Maximum Speed (Real-time screening, high patient volume)',
            'efficiency': '💰 Maximum Efficiency (Mobile/Edge deployment, low compute power)',
            'balanced': '⚖️ Balanced Performance (All-around clinical use)'
        }
    ...
    if submitted:
        ...
        for i, row in recommendations_raw.iterrows():
            ...
            st.markdown(f"### 🏆 #{i+1}: {row['architecture']}")
